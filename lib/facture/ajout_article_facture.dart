...
class Ajout_article_facture extends StatefulWidget {


  @override
  Ajout_article_factureState createState() => Ajout_article_factureState();
}

class Ajout_article_factureState extends State<Ajout_article_facture> {
          ...
          IconButton(
            ...
            onPressed: () {
              if (_formKey.currentState!.validate() == true) {
                ...
                if(provFacture.existArticleInFacture(art.id)!=-1)

                {
                  provFacture.updateArticleInListArticle({art : provFacture.listArticle[provFacture.existArticleInFacture(art.id)].values.first + quantite}, provFacture.existArticleInFacture(art.id));
                  provlistArticle.updateItem(art, provlistArticle.existArticle(art.id));

                }
                else{
                  provFacture.addArticleToListArticle({art : quantite});
